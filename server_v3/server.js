...
if (process.env.NODE_ENV === 'sakura') {
  require('dotenv').config({ path: '.env.sakura' });
}
...
if (ENABLE_HTTPS) {
  try {
    ...
    console.log('服务器以HTTPS模式启动');
  } catch (error) {
    console.error('HTTPS服务器创建失败，将回退到HTTP模式', error);
    server = http.createServer(app);
  }
} else {
  server = http.createServer(app);
  console.log('服务器以HTTP模式启动');
}
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Server is accessible at http://localhost:${PORT} or http://[your-local-ip]:${PORT}`);
});
